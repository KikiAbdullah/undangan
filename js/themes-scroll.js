(() => {
  ...
  var r = document.getElementById("towerBarat"),
    d = document.querySelector(".towerbarat_cover");
  d.style.position = "fixed";
  var c = document.documentElement.lang,
    u = document.getElementById("loader");
  u &&
    ((window.onload = u.style.display = "none"),
    (window.onload = r.style.display = "inherit"));
  ...
  (playMusic = function (e) {
    v && (e ? (v.play(), (w = !0)) : (w ? v.pause() : v.play(), (w = !w))),
      ...
      g && (w ? g.classList.add("playing") : g.classList.remove("playing"));
  }),
    v &&
    ...
    (showGift = function (e) {
      for (var t = 0; t < E.length; t++)
        t != e && (L[t].style.display = "none");
      L[e].style.display = "inherit";
    });
  for (
    var L = document.getElementsByClassName("gift-container"), x = 0;
    x < L.length;
    x++
  )
  ...
  (showLightbox = function (e) {
    B.classList.add("show"),
      (I.innerHTML = '<div class="lightbox-inner"><img src="'.concat(
        S[e].src,
        '"></div>'
      )),
      (C.dataset.index = e),
  ...
  for (
    var V = document.getElementsByClassName("copy-address"), X = 0;
    X < V.length;
    X++
  )
    V[X].innerHTML &&
      ("id" == c
        ? V[X].insertAdjacentHTML(
            "afterend",
  ...
  var Z = document.querySelectorAll(".towerbarat_slide");
  Array.from(Z).forEach(function (e) {
    e.classList.contains("towerbarat_cover") || (e.style.display = "none");
  });
  var $ = document.querySelectorAll(".animate__animated"),
    ee = function () {
      window.scrollY;
      Array.from($).forEach(function (e) {
        e.getBoundingClientRect().y - window.innerHeight < 0 &&
          ("hidden" == e.style.visibility && (e.style.display = "none"),
          setTimeout(function () {
  ...
  for (
    var te = function (e) {
        ...
        playMusic(!0),
          ...
          ee(),
          null === (t = document.querySelector(".not-open")) ||
            void 0 === t ||
            t.classList.remove("not-open"),
          e.target.remove();
      },
      ne = document.getElementsByClassName("btn-open-invitation"),
      oe = 0;
    oe < ne.length;
    oe++
  )
    ne[oe].addEventListener("click", te, !1);
})();
